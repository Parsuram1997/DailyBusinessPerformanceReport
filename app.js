...
function initGlobalSettings() {
    ...
    globalSettingsUnsubscribe = onSnapshot(settingsRef, (docSnap) => {
        if (docSnap.exists()) {
            ...
            checkAndSet('dtxn_showBalancesGrid', data.dtxn_showBalancesGrid);
            ...
            checkAndSet('validate_txns_checked', data.validate_txns_checked);
            checkAndSet('validate_cash_diff_limit', data.validate_cash_diff_limit);
            checkAndSet('validate_online_diff_limit', data.validate_online_diff_limit);
            checkAndSet('validate_csp_diff_limit', data.validate_csp_diff_limit);
            ...
            checkAndSet('user_validate_txns_checked', data.user_validate_txns_checked);
...
// Helper: get validation diff limit for a given type (cash/online/csp), defaulting to 2000
function getValidationLimit(type) {
    const key = `validate_${type}_diff_limit`;
    const stored = localStorage.getItem(key);
    const val = stored !== null ? parseInt(stored) : NaN;
...
async function initAddEntry() {
    ...
    const lockAutoSyncedFields = (isExisting = false) => {
        ...
        autoSyncIds.forEach(id => {
            ...
            if (input) {
                ...
                // For expense: sync icon goes right-2 (overlaps with button area, both are small)
                // For others: sync icon at right-3
                let rightPos = (button && id === 'expense') ? 'right-9' : 'right-3';
                // Reduce padding: expense only needs room for small sync icon + split button (64px), others 40px
                ...
                if (isValidate) {
                    ...
                    for (const inputId of inputs) {
                        ...
                        if (input && input.value !== '' && expectedEl && expectedEl.dataset.val !== undefined) {
                            const entered = parseFloat(input.value) || 0;
                            const expected = parseFloat(expectedEl.dataset.val) || 0;
                            if (Math.abs(entered - expected) > getValidationLimit('online')) {
                                hasExceeded = true;
                                break;
                            }
                        }
                    ...
                    if (hasExceeded) {
                        if (typeof Swal !== 'undefined') {
                            Swal.fire({
                                ...
                                text: `The difference for one of the individual Online accounts exceeds ₹${getValidationLimit('online').toLocaleString('en-IN')}. Please verify and correct your transactions on the Daily Txn page.`,
                                icon: 'error',
                                confirmButtonColor: '#e11d48'
                            });
                        } else {
                            alert(`The difference for one of the individual Online accounts exceeds ₹${getValidationLimit('online').toLocaleString('en-IN')}. Please verify and correct your transactions.`);
                        }
                        return;
                    }
                }
                ...
                if (isValidate) {
                    ...
                    for (const inputId of inputs) {
                        ...
                        if (input && input.value !== '' && expectedEl && expectedEl.dataset.val !== undefined) {
                            const entered = parseFloat(input.value) || 0;
                            const expected = parseFloat(expectedEl.dataset.val) || 0;
                            if (Math.abs(entered - expected) > getValidationLimit('csp')) {
                                hasExceeded = true;
                                break;
                            }
                        }
                    ...
                    if (hasExceeded) {
                        if (typeof Swal !== 'undefined') {
                            Swal.fire({
                                title: 'Validation Error',
                                text: `The difference for one of the individual CSP Wallet accounts exceeds ₹${getValidationLimit('csp').toLocaleString('en-IN')}. Please verify and correct your transactions on the Daily Txn page.`,
                                icon: 'error',
                                confirmButtonColor: '#e11d48'
                            });
                        } else {
                            alert(`The difference for one of the individual CSP Wallet accounts exceeds ₹${getValidationLimit('csp').toLocaleString('en-IN')}. Please verify and correct your transactions.`);
                        }
                        return;
                    }
                }
    ...
    (() => {
        ...
        const modalUpdateTotals = () => {
            ...
            if (modalDatePicker && modalDatePicker.value) saveCashCalculator(counts, modalDatePicker.value);
        };
        ...
        const modalLoadForDate = async (dateStr) => {
            ...
            let savedCounts = {};
            try { savedCounts = await loadCashCalculator(dateStr); delete savedCounts.date; } catch(e) {}
        ...
        if (modalDatePicker) modalDatePicker.addEventListener('change', () => modalLoadForDate(modalDatePicker.value));
        ...
        if (modalUseCashBtn) {
            modalUseCashBtn.addEventListener('click', () => {
                ...
                if (finalAmount > 0) {
                    ...
                    if (isValidate && Math.abs(finalAmount - modalCurrentSystemCash) > getValidationLimit('cash')) {
                        if (typeof Swal !== 'undefined') {
                            Swal.fire({ title: 'Validation Error', text: `The difference between Expected Cash and your manual physical total exceeds ₹${getValidationLimit('cash').toLocaleString('en-IN')}. Please verify your Daily Txn transactions.`, icon: 'error', confirmButtonColor: '#e11d48' });
                        } else { alert(`Cash difference exceeds ₹${getValidationLimit('cash').toLocaleString('en-IN')}.`); }
                        return;
                    }
                    if (cashInput) {
                    ...
                    } else { alert('Please calculate an amount greater than 0.'); }
                }
            });
        }
        ...
        try {
            ...
            const checkDifferences = (inputIds, settingKey) => {
                ...
                for (let i = 0; i < inputIds.length; i++) {
                    ...
                    if (inp && inp.value !== '' && exp && exp.dataset && exp.dataset.val != null) {
                        ...
                        if (diff > getValidationLimit(settingKey === 'validate_online_diff' ? 'online' : 'csp')) {
                            hasValidationError = true;
                            const name = nameMap[inputIds[i]] || inputIds[i].replace(/_/g, ' ');
                            const lim = getValidationLimit(settingKey === 'validate_online_diff' ? 'online' : 'csp');
                            validationErrorMsg = name + ' individual difference is Rs ' + diff.toLocaleString('en-IN') + ' which exceeds the Rs ' + lim.toLocaleString('en-IN') + ' limit.';
                            break;
                        }
                    }
                }
            ...
            const checkMainDifference = (inputId, expId, name, settingKey) => {
                 ...
                 if (inp && inp.value !== '' && exp && exp.dataset && exp.dataset.val != null) {
                        ...
                        const expected = parseFloat(exp.dataset.val) || 0;
                        const diff = Math.abs(entered - expected);
                        const limType = settingKey === 'validate_online_diff' ? 'online' : 'csp';
                        if (diff > getValidationLimit(limType)) {
                            hasValidationError = true;
                            validationErrorMsg = name + ' manual total and Expected ' + name + ' difference is Rs ' + diff.toLocaleString('en-IN') + ' which exceeds the Rs ' + getValidationLimit(limType).toLocaleString('en-IN') + ' limit.';
                        }
                 }
            };
            ...
            if (!hasValidationError && (window.getAppSetting ? window.getAppSetting('validate_online_diff', true) : (localStorage.getItem('validate_online_diff') !== 'false'))) {
                ...
                if (typeof currentSystemOnline !== 'undefined') {
                    ...
                    if (diff > getValidationLimit('online')) {
                        hasValidationError = true;
                        validationErrorMsg = 'Overall Online Accounts (Online+CSP+Jio+CRGB+Pending) total difference is Rs ' + diff.toLocaleString('en-IN') + ' which exceeds the Rs ' + getValidationLimit('online').toLocaleString('en-IN') + ' limit.';
                    }
                }
            }
            ...
            if (!hasValidationError && (window.getAppSetting ? window.getAppSetting('validate_cash_diff', true) : (localStorage.getItem('validate_cash_diff') !== 'false'))) {
                ...
                if (window._roinetBreakdown && window._roinetBreakdown.cash) {
                    ...
                    if (diff > getValidationLimit('cash')) {
                        hasValidationError = true;
                        validationErrorMsg = 'Cash difference is Rs ' + diff.toLocaleString('en-IN') + ' which exceeds the Rs ' + getValidationLimit('cash').toLocaleString('en-IN') + ' limit.';
                    }
                }
            }
            ...
            if (isValidateTxnsChecked) {
                ...
                if (dateVal) {
                    ...
                    const uncheckedTxns = [];
                    const typesToCheck = ['AEPS', 'MATM', 'DEPOSIT', 'WITHDRAWAL', 'QR_WITHDRAWAL', 'AADHAAR_PAY'];
...
async function initCalculator() {
    ...
    let currentSystemCash = 0;

    const updateSystemComparison = (physicalTotal) => {
        const sysCashEl = document.getElementById('system-cash-val');
        const diffEl = document.getElementById('cash-diff-val');

        if (sysCashEl) sysCashEl.innerText = formatCurrency(currentSystemCash);

        if (diffEl) {
            const diff = (physicalTotal || 0) - currentSystemCash;
            diffEl.innerText = formatCurrency(diff);

            // Color coding
            if (Math.abs(diff) < 0.01) {
                diffEl.className = 'font-bold italic text-white';
            } else if (diff > 0) {
                diffEl.className = 'font-bold italic text-emerald-300';
            } else {
                diffEl.className = 'font-bold italic text-rose-300';
            }
        }
    };

    const fetchSystemCash = async (dateStr) => {
        try {
            const [yr, mo, dy] = dateStr.split('-');
            const current = new Date(yr, mo - 1, dy);
            const yesterday = new Date(current);
            ...
            console.error("Error fetching system cash:", e);
            return 0;
        }
    };
    ...
    const updateTotals = () => {
        ...
        updateSystemComparison(grandTotal);
    ...
    if (!calcDatePicker && headerFlex) {
        ...
        pickerWrapper.className = 'flex flex-col gap-1 w-full md:w-auto min-w-0 md:min-w-[180px]';
        pickerWrapper.innerHTML = `
            <label class="text-[10px] lg:text-xs font-bold text-slate-500 uppercase tracking-wider">Select Date</label>
            <input id="calc-date-picker" type="date"
                class="px-3 lg:px-4 py-2 lg:py-2.5 rounded-lg border border-primary/20 bg-white dark:bg-background-dark/40 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-xs lg:text-sm font-semibold text-slate-700 dark:text-slate-200 w-full">
        `;
    ...
    const loadForDate = async (dateStr) => {
        currentSystemCash = await fetchSystemCash(dateStr);
    ...
    if (btnUseCash) {
            ...
            if (finalAmount > 0) {
                ...
                if (isValidate && typeof currentSystemCash !== 'undefined') {
                    if (Math.abs(finalAmount - currentSystemCash) > getValidationLimit('cash')) {
                        if (typeof Swal !== 'undefined') {
                            Swal.fire({
                                title: 'Validation Error',
                                text: `The difference between Expected Cash and your manual physical total exceeds ₹${getValidationLimit('cash').toLocaleString('en-IN')}. Please verify and correct your transactions on the Daily Txn page.`,
                                icon: 'error',
                                confirmButtonColor: '#e11d48'
                            });
                        } else {
                            alert(`The difference between Expected Cash and your manual physical total exceeds ₹${getValidationLimit('cash').toLocaleString('en-IN')}. Please verify and correct your transactions on the Daily Txn page.`);
                        }
                        return;
                    }
                }
...
window.goToCashCalculator = () => {
    ...
    if (window._openCashCalculatorModal) {
        window._openCashCalculatorModal();
    } else {
...
async function initReports() {
    ...
    async function renderReport() {
        ...
        if (reportsSummaryArea) {
            ...
            const includedVolumeTypes = ['AEPS', 'MATM', 'DEPOSIT', 'AADHAAR_DEPOSIT', 'WITHDRAWAL', 'QR_WITHDRAWAL', 'AADHAAR_PAY'];
        ...
        // Render Top Customers Table (Top 20 Repeated)
        const topCustomersBody = document.getElementById('top-customers-body');
        if (topCustomersBody) {
            const customerCounts = {};
            const nonCustomerTypes = ['DAILY_EXPENSE', 'SETTLEMENT', 'GOLD_SIP', 'CSP_COMMISSION', 'ADD_CAPITAL', 'SHARE_WITHDRAWN', 'CASH_WITHDRAWAL', 'CASH_DEPOSIT', 'PENDING_ADD', 'PENDING_REMOVE'];
            
            dailyTxns.forEach(t => {
                if (t.note && t.note.trim() !== '' && !nonCustomerTypes.includes(t.type) && parseFloat(t.charges || 0) > 0) {
                    // Normalize the customer name to group effectively
                    const rawName = t.note.trim().toLowerCase();
                    const nameWords = rawName.split(/\s+/).map(w => w.charAt(0).toUpperCase() + w.slice(1));
                    const normalizedName = nameWords.join(' ');
                    
                    // Normalize the address/village
                    const rawAddress = (t.address || '').trim().toLowerCase();
                    const addressWords = rawAddress.split(/\s+/).map(w => w.charAt(0).toUpperCase() + w.slice(1));
                    const normalizedAddress = addressWords.join(' ');
                    ...
                    customerCounts[key].volume += amt;
                    customerCounts[key].fees += chg;
                }
            });
            
            // Sort by visits/transactions count descending
            ...
            if (top20Customers.length === 0) {
                ...
                topCustomersBody.innerHTML = top20Customers.map((c, idx) => `
                    <tr class="hover:bg-primary/5 transition-colors group">
                        <td class="px-4 py-2 font-bold text-slate-500 w-12 text-center">${idx + 1}</td>
                        <td class="px-4 py-2 font-semibold text-slate-700 dark:text-slate-200">
                            ${c.name}
                        </td>
                        <td class="px-4 py-2 text-slate-500 dark:text-slate-400 font-medium">
                            ${c.address || '<span class="text-slate-400 italic text-xs">Not Specified</span>'}
                        </td>
                        <td class="px-4 py-2 text-right font-black text-indigo-600 dark:text-indigo-400">
                            ${c.count}
                        </td>
                        <td class="px-4 py-2 text-right font-bold text-slate-600 dark:text-slate-300">
        ...
        if (amountDistCanvas && amountDistTable && typeof Chart !== 'undefined') {
            const distBins = [
                { label: '₹1 - ₹500', min: 1, max: 500, count: 0, total: 0, fees: 0, color: '#f43f5e' },
                { label: '₹501 - ₹1,000', min: 501, max: 1000, count: 0, total: 0, fees: 0, color: '#fb7185' },
            ...
            const excludedDistTypes = ['DAILY_EXPENSE', 'SETTLEMENT', 'GOLD_SIP', 'CSP_COMMISSION', 'ADD_CAPITAL', 'SHARE_WITHDRAWN', 'PENDING_ADD', 'PENDING_REMOVE'];
        ...
        const reconDates = [...new Set(filtered.map(e => e.date))]
            .filter(d => systemDates.has(d) && d >= '2026-05-01');

        let totalManualCash = 0;
        let totalSysCash = 0;
        let totalManualOnline = 0;
        let totalSysOnline = 0;
        let totalManualIncome = 0;
        let totalSysIncome = 0;

        reconDates.forEach(date => {
            const e = filtered.find(ent => ent.date === date);
            const det = e.details || {};
            const manualCash = parseFloat(det.cash || 0);
            const manualOnline = (parseFloat(det.online || 0) +
                parseFloat(det.roinet || 0) +
                parseFloat(det.go2sms || 0) +
                parseFloat(det.jio || 0) +
                parseFloat(det.pending || 0));

            // Use the pre-calculated daily income from the map
            const mapped = dailyIncomeMap[date] || {};
            totalManualIncome += (mapped.inc || 0);
        ...
        const finalOnlineDiff = totalManualOnline - totalSysOnline;
        const finalIncomeDiff = totalManualIncome - totalSysIncome;
    ...
    <div id="pin-modal" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);display:flex;align-items:center;justify-content:center;z-index:99999;backdrop-filter:blur(10px);">
        ...
        <div style="background:white;padding:32px;border-radius:24px;box-shadow:0 20px 50px rgba(0,0,0,0.3);text-align:center;width:90%;max-width:360px;border:1px solid ${accentColor}33;" class="dark:bg-slate-900 dark:border-primary/30">
            ...
            <div style="display:flex;gap:8px;justify-content:center;margin-bottom:32px;" id="pin-container">
                ${[1, 2, 3, 4, 5, 6].map(() => `
                    <input type="password" class="pin-digit dark:bg-slate-800 dark:border-slate-700 dark:text-white focus:border-primary" style="width:42px;height:52px;font-size:24px;text-align:center;border:2px solid #e2e8f0;border-radius:12px;outline:none;transition:all 0.2s;" maxlength="1" inputmode="numeric">
                `).join('')}
            </div>
    ...
    const attemptUnlock = () => {
        ...
        if (pin === "202526") {
            ...
            inputs.forEach(i => {
                ...
                i.style.background = "rgba(239, 68, 68, 0.05)";
                i.value = "";
            });
            inputs[0].focus();
            setTimeout(() => {
...
function protectPrivilegedLinks() {
    const selectors = [
        ...
        'a[href="daily-txn.html"]',
        'a[data-page="daily-txn.html"]'
    ].join(', ');
...
const MASTER_TXN_TYPES = [
    ...
    { type: 'ONLINE_EXCHANGE', label: 'Online Exchange', icon: 'currency_exchange', bg: 'bg-indigo-50 dark:bg-indigo-500/10', text: 'text-indigo-600 dark:text-indigo-400', borderLeft: 'bg-indigo-500' }
];
...
async function initDailyTxn() {
    ...
    if (!isLoggedIn) {
        ...
        if (!isUnlocked && pinEnabled) {
            ...
            window.showSecurityPIN('Daily Transactions', () => {
                sessionStorage.setItem('dailyTxnUnlocked', 'true');
                if (mainContent) {
                    mainContent.style.opacity = '1';
                    mainContent.style.transition = 'opacity 0.5s ease';
                }
            }, () => {
                // If cancelled, go back to login
    ...
    const updateConditionalField = () => {
        ...
        const chargesOnlyTypes = ['PHOTOCOPY', 'PRINTOUT', 'PASSPORT', 'LAMINATION', 'CSP_COMMISSION', 'OTHER_INCOME'];
        ...
        if (isChargesOnly || isSimplified || isAmountOnly || isNoteAndAmount || isCredit || isDamagedRecovery || isCapital || isPending || isCashMovement) {
            conditionalContainer.classList.add('hidden');
            if (amountFieldContainer) {
                if (isSimplified || isAmountOnly || isNoteAndAmount || isCredit || isDamagedRecovery || isCapital || isPending || isCashMovement) amountFieldContainer.classList.remove('hidden');
        ...
        // Set Default Charges Mode based on Type ONLY when txnType actually changes
        window._lastTxnType = window._lastTxnType || '';
        if (txnType.value !== window._lastTxnType) {
            ...
            window._lastTxnType = txnType.value;
        }
        ...
        if (providerTypes.includes(txnType.value)) {
            ...
            if (['FREE_DEPOSIT', 'FREE_WITHDRAWAL'].includes(txnType.value)) {
                ...
                if (amountLabel) amountLabel.innerText = 'Amount';
            } else if (txnType.value === 'ONLINE_EXCHANGE') {
                if (providerLabel) providerLabel.innerText = 'Source Account';
                if (providerFirstOpt) providerFirstOpt.innerText = 'Select Source...';
                if (amountLabel) amountLabel.innerText = 'Amount';
            } else if (isCredit || ['DAMAGED_RECOVERY', 'ADD_CAPITAL', 'SHARE_WITHDRAWN', 'ONLINE_WORK', 'JIO_RECHARGE', 'JIO_TOPUP'].includes(txnType.value)) {
                ...
                if (amountLabel) amountLabel.innerText = txnType.value === 'ONLINE_WORK' ? 'Txn Amount' : (['JIO_RECHARGE', 'JIO_TOPUP'].includes(txnType.value) ? 'Recharge Amount' : 'Amount');
        ...
        if (['QR_WITHDRAWAL', 'ONLINE_EXCHANGE'].includes(txnType.value)) {
            qrWalletContainer.classList.remove('hidden');
            const qrLabel = document.getElementById('qrwallet-label');
            if (qrLabel) qrLabel.innerText = txnType.value === 'ONLINE_EXCHANGE' ? 'Destination Account' : 'QR Wallet';
        } else {
            ...
            if (['ONLINE_WORK', 'DEPOSIT', 'WITHDRAWAL', 'FREE_DEPOSIT', 'FREE_WITHDRAWAL', 'SETTLEMENT', 'CASH_WITHDRAWAL', 'CASH_DEPOSIT', 'PENDING_ADD', 'PENDING_REMOVE', 'GOLD_SIP'].includes(txnType.value) || isCustMoneyOnline) {
                ...
                if (depositByLabel) {
                    if (txnType.value === 'ONLINE_WORK') {
                        ...
                        depositByLabel.innerText = 'Deposit By';
                    }
                }
            } else {
        ...
        if (txnProvider) {
            ...
            Array.from(txnProvider.options).forEach(opt => {
                ...
                if (txnType.value === 'FREE_DEPOSIT') {
                    ...
                    opt.style.display = ['online(parsu)', 'online(shop)', 'online(dalai)'].includes(opt.value) ? '' : 'none';
                } else if (isAepsMatm) {
                ...
                } else if (isCredit || ['JIO_RECHARGE', 'JIO_TOPUP', 'ONLINE_WORK', 'DAMAGED_RECOVERY', 'ADD_CAPITAL', 'SHARE_WITHDRAWN'].includes(txnType.value)) {
        ...
        if (['QR_WITHDRAWAL', 'ONLINE_EXCHANGE'].includes(txnType.value)) {
            const qrWallets = ['Airtel(Parsu)', 'Airtel(Dalai)', 'Roinet(Parsu)', 'Roinet(Dalai)', 'SpiceMoney'];
            const onlineWallets = ['online(parsu)', 'online(shop)', 'online(dalai)'];
            Array.from(txnQrWallet.options).forEach(opt => {
                if (!opt.value) return;
                if (txnType.value === 'ONLINE_EXCHANGE') {
                    opt.style.display = onlineWallets.includes(opt.value) ? '' : 'none';
                } else if (txnType.value === 'QR_WITHDRAWAL') {
                    opt.style.display = qrWallets.includes(opt.value) ? '' : 'none';
                }
            });
            if (txnQrWallet.selectedOptions[0] && txnQrWallet.selectedOptions[0].style.display === 'none') {
                txnQrWallet.value = '';
            }
        }
        ...
        if (txnReason) txnReason.tabIndex = 2;
    ...
    if (txnProvider) {
        txnProvider.addEventListener('change', updateConditionalField);
    }
    if (txnChargesType) {
        txnChargesType.addEventListener('change', updateConditionalField);
    }
    updateConditionalField();
    if (txnType) {
        txnType.focus();
    ...
    form.onsubmit = async (e) => {
        ...
        try {
            ...
            if (window._onlineBreakdown && window._onlineBreakdown.closing) {
                const getOnlineSub = (prov) => {
                    const l = (prov || '').toLowerCase();
                    if (l.includes('parsu')) return 'online_p1';
                    if (l.includes('shop')) return 'online_p2';
                    if (l.includes('dalai')) return 'online_p3';
                    return 'other';
                };
                let deductOnlineAccount = null;
                let deductOnlineAmount = 0;
                let pName = '';

                if (newTxn.type === 'ONLINE_WORK') {
                    ...
                    deductOnlineAmount = newTxn.amount;
                    pName = newTxn.depositBy || 'Online';
                } else if (newTxn.type === 'CASH_WITHDRAWAL') {
                    deductOnlineAccount = getOnlineSub(newTxn.depositBy);
                    deductOnlineAmount = newTxn.amount;
                    pName = newTxn.depositBy || 'Online';
                ...
                if (deductOnlineAccount) {
                    ...
                    if (deductOnlineAmount > available) {
                        Swal.fire({
                            ...
                            text: `Account ${pName} only has ₹ ${available.toLocaleString('en-IN')}. You cannot deduct ₹ ${deductOnlineAmount.toLocaleString('en-IN')}.`,
                            confirmButtonColor: '#7c3aed'
                        });
                        if (submitBtn) {
                            submitBtn.disabled = false;
            ...
            // Generic Closing Balance validations for CSP Wallets
            if (window._roinetBreakdown && window._roinetBreakdown.closing) {
                const getRoinetSub = (prov) => {
                    const l = (prov || '').toLowerCase();
                    if (l.includes('roinet(parsu)') || l === 'roinet_1') return 'roinet_1';
                    if (l.includes('roinet(dalai)') || l === 'roinet_2') return 'roinet_2';
                    if (l.includes('airtel(parsu)') || l === 'airtel_1') return 'airtel_1';
                    if (l.includes('airtel(dalai)') || l === 'airtel_2') return 'airtel_2';
                    if (l.includes('spicemoney')) return 'spicemoney';
                    if (l.includes('airtel')) return 'airtel_1';
                    if (l.includes('roinet')) return 'roinet_1';
                    return null;
                };

                let deductRoinetAccount = null;
                let deductRoinetAmount = 0;
                let pName = '';

                if (newTxn.type === 'CSP_SUBSCRIPTION') {
                ...
                if (deductRoinetAccount) {
                    ...
                    if (deductRoinetAmount > available) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Insufficient CSP Balance',
                            text: `Account ${pName} only has ₹ ${available.toLocaleString('en-IN')}. You cannot deduct ₹ ${deductRoinetAmount.toLocaleString('en-IN')}.`,
                            confirmButtonColor: '#7c3aed'
                        });
                        if (submitBtn) {
                            submitBtn.disabled = false;
            ...
            if (['AEPS', 'MATM', 'DEPOSIT', 'AADHAAR_DEPOSIT', 'WITHDRAWAL', 'QR_WITHDRAWAL', 'AADHAAR_PAY'].includes(newTxn.type)) {
    ...
    const updateDailyBalances = async (date, txns = []) => {
        ...
        try {
            ...
            sortedTxns.forEach(t => {
                ...
                if (['AEPS', 'MATM', 'WITHDRAWAL', 'QR_WITHDRAWAL', 'FREE_WITHDRAWAL', 'ADMIN_WITHDRAWAL'].includes(t.type)) {
                    ...
                    if (provider.includes('crgb')) {
                        ...
                        balances.roinet += chg;
                        updateRoinetDelta(provider, chg);
                    }
                } else if (t.type === 'GOLD_SIP') {
                    balances.online -= amt;
                    balances.expense += amt;
                    expenseBreakdown['GOLD SIP'] = (expenseBreakdown['GOLD SIP'] || 0) + amt;
                    onlineBreakdown[getOnlineDest(t.depositBy || 'shop')] -= amt;
                } else if (t.type === 'CREDIT_GIVEN') {
                    if (provider === 'cash') balances.cash -= amt;
                    else {
                        balances.online -= amt;
                    ...
                    if (provider === 'cash') balances.cash += amt;
                        ...
                        balances.online += amt;
                        onlineBreakdown[getOnlineDest(t.depositBy || provider)] += amt;
                        if (provider.includes('roinet') || provider.includes('airtel') || provider.includes('spicemoney')) balances.roinet += amt;
                        else if (provider.includes('crgb')) balances.crgb += amt;
                        else if (provider.includes('jio')) balances.jio += amt;
                    }
            ...
            const opRoinet1 = parseFloat(details.roinet_1 || 0);
            const opRoinet2 = parseFloat(details.roinet_2 || 0);
            const opAirtel1 = parseFloat(details.airtel_1 || 0);
            const opAirtel2 = parseFloat(details.airtel_2 || 0);
            const opSpiceMoney = parseFloat(details.spicemoney || 0);
            const opRoinet = opRoinet1 + opRoinet2;
            const opAirtel = opAirtel1 + opAirtel2;
            ...
            window._roinetBreakdown = {
                opening: {
                    roinet_1: totalSplitOpening > 0 ? opRoinet1 : roinetOpeningFallback,
                    roinet_2: opRoinet2,
                    airtel_1: opAirtel1,
                    airtel_2: opAirtel2,
                ...
                closing: {
                    roinet_1: (totalSplitOpening > 0 ? opRoinet1 : roinetOpeningFallback) + roinetBreakdown.roinet_1,
                    roinet_2: opRoinet2 + roinetBreakdown.roinet_2,
                    airtel_1: opAirtel1 + roinetBreakdown.airtel_1,
                    airtel_2: opAirtel2 + roinetBreakdown.airtel_2,
    ...
    const renderBadgesAndTable = () => {
        ...
        // Update detail cards (if they exist in server DOM)
                ...
                <td class="px-3 py-2 text-right">
                    <div class="flex flex-col items-center justify-center gap-1 min-w-[100px]">
                        <span class="text-[10px] font-black text-emerald-500/70 px-2 py-0.5 rounded border border-emerald-500/10 w-full flex justify-between items-center">
                            <span>C: ₹${currentStartCash.toLocaleString('en-IN')}</span>
                        </span>
                        <span class="text-[10px] font-black text-blue-500/70 px-2 py-0.5 rounded border border-blue-500/10 w-full flex justify-between items-center">
                            <span>O: ₹${currentStartOnline.toLocaleString('en-IN')}</span>
                        </span>
                    </div>
                </td>
        ...
        document.querySelectorAll('.edit-txn-btn').forEach(btn => {
            btn.onclick = () => {
                ...
                if (txn) {
                    ...
                    if (txnCharges) txnCharges.value = txn.charges;
                    ...
                    if (txnNote) txnNote.value = txn.note;
                    ...
                    if (['QR_WITHDRAWAL', 'ONLINE_EXCHANGE'].includes(txn.type)) {
                    ...
                    if (txnReason && txn.reason) txnReason.value = txn.reason;
    ...
    const loadTransactions = (date) => {
        try {
            ...
            updateDailyBalances(date, []);

            const dateQueries = getPossibleDateFormats(date);
            console.log("Querying transactions for formats:", dateQueries);
            ...
            unsubscribe = onSnapshot(q, async (snapshot) => {
                ...
                // Calculate running balances: Sort chronologically (Oldest first)
                ...
                txns = txns.map(t => {
                    ...
                    if (['AEPS', 'MATM', 'WITHDRAWAL', 'QR_WITHDRAWAL', 'FREE_WITHDRAWAL', 'ADMIN_WITHDRAWAL'].includes(t.type)) {
                        ...
                        const provCharge = parseFloat(t.providerCharge || 0);
                        currentCash -= amt;
                        addAmt(provider, amt - provCharge);
                    } else if (['DEPOSIT', 'AADHAAR_DEPOSIT', 'FREE_DEPOSIT', 'ADMIN_DEPOSIT'].includes(t.type)) {
                        currentCash += amt;
                        addAmt(provider, -amt);
                    } else if (['DISHTV_RECHARGE', 'ELECTRICITY_BILL', 'PAN_CARD'].includes(t.type)) {
                    ...
                    } else if (['CSP_COMMISSION', 'OTHER_INCOME'].includes(t.type)) {
                        ...
                        if (t.depositBy) addAmt(t.depositBy, -amt);
                        else addAmt('shop', -amt);
                    } else if (t.type === 'CREDIT_GIVEN') {
                    ...
                    tableBody.innerHTML = `
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center text-rose-500 font-bold border-2 border-rose-100 rounded-xl bg-rose-50/50">
                                ...
                                <div class="mt-4 text-[10px] text-slate-500 font-normal">If you are using a local file, please run 'node server.js' first.</div>
                            </td>
                        </tr>
                    `;
                }
    ...
    if (downloadPdfBtn) {
        downloadPdfBtn.onclick = () => {
            ...
            try {
                ...
                doc.setFontSize(18);
                doc.text(`Daily Transaction Report - ${currentSelectedDate}`, 14, 15);
                doc.setFontSize(10);
                ...
                const fileNameDate = window.isAllTimeSearchMode ? "All_Time" : currentSelectedDate;
                doc.save(`Transactions_${fileNameDate}.pdf`);
            } catch (err) {
    ...
    if (downloadExcelBtn) {
        downloadExcelBtn.onclick = () => {
            ...
            try {
                ...
                window.XLSX.utils.book_append_sheet(wb, ws, "Daily Transactions");
                const fileNameDate = window.isAllTimeSearchMode ? "All_Time" : currentSelectedDate;
                window.XLSX.writeFile(wb, `Transactions_${fileNameDate}.xlsx`);
            } catch (err) {
