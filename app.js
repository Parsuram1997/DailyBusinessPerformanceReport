...
async function initDailyTxn() {
    ...
    const updateConditionalField = () => {
        ...
        const providerTypes = ['AEPS', 'MATM', 'DEPOSIT', 'AADHAAR_DEPOSIT', 'WITHDRAWAL', 'QR_WITHDRAWAL', 'FREE_DEPOSIT', 'FREE_WITHDRAWAL', 'CREDIT_GIVEN', 'CREDIT_RECEIVED', 'DISHTV_RECHARGE', 'JIO_RECHARGE', 'ELECTRICITY_BILL', 'PAN_CARD', 'CUST_MONEY_IN', 'CUST_MONEY_OUT', 'DAILY_EXPENSE', 'SETTLEMENT', 'ONLINE_WORK', 'DAMAGED_RECOVERY', 'ADD_CAPITAL', 'SHARE_WITHDRAWN', 'CSP_COMMISSION', 'ONLINE_EXCHANGE'];
        ...
        if (depositByContainer && txnDepositBy) {
            const isCustMoneyOnline = ['CUST_MONEY_IN', 'CUST_MONEY_OUT', 'CREDIT_GIVEN', 'CREDIT_RECEIVED'].includes(txnType.value) && txnProvider && txnProvider.value === 'Online';
            if (['DEPOSIT', 'WITHDRAWAL', 'FREE_DEPOSIT', 'FREE_WITHDRAWAL', 'SETTLEMENT'].includes(txnType.value) || isCustMoneyOnline) {
                ...
                if (depositByLabel) {
                    if (['WITHDRAWAL', 'FREE_WITHDRAWAL', 'SETTLEMENT', 'CUST_MONEY_IN', 'CREDIT_RECEIVED'].includes(txnType.value)) {
                        depositByLabel.innerText = 'Received In';
                    } else if (['CUST_MONEY_OUT', 'CREDIT_GIVEN'].includes(txnType.value)) {
    ...
    form.onsubmit = async (e) => {
        ...
        try {
            ...
            const newTxn = {
                ...
                depositBy: (['DEPOSIT', 'WITHDRAWAL', 'FREE_DEPOSIT', 'FREE_WITHDRAWAL', 'QR_WITHDRAWAL', 'SETTLEMENT'].includes(txnType.value) || (['CUST_MONEY_IN', 'CUST_MONEY_OUT', 'CREDIT_GIVEN', 'CREDIT_RECEIVED'].includes(txnType.value) && txnProvider && txnProvider.value === 'Online')) ? (txnDepositBy ? txnDepositBy.value : '') : '',
                ...
                Object.keys(balances).forEach(key => {
                    ...
                    if (diff !== 0) {
                        ...
                        if (key === 'online') {
                            ...
                            const actualProviderForOnline = (['CUST_MONEY_IN', 'CUST_MONEY_OUT', 'CREDIT_GIVEN', 'CREDIT_RECEIVED'].includes(t.type)) && provider === 'online' ? (t.depositBy || provider) : provider;
                    ...
                    <div class="flex flex-col gap-1.5 max-w-[200px]">
                        ...
                        ${txn.depositBy ? `
                            <div class="flex items-center gap-1.5 px-2 py-1 rounded bg-purple-50 dark:bg-purple-500/10 border border-purple-100 dark:border-purple-500/20 w-fit">
                                ...
                                <span class="text-[10px] font-bold text-purple-700 dark:text-purple-400 tracking-wide">${['WITHDRAWAL', 'FREE_WITHDRAWAL', 'SETTLEMENT', 'CUST_MONEY_IN', 'CREDIT_RECEIVED'].includes(txn.type) ? 'Recv:' : (['CUST_MONEY_OUT', 'CREDIT_GIVEN'].includes(txn.type) ? 'Debit:' : 'Dep:')} ${txn.depositBy.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, c => c.toUpperCase())}</span>
                            </div>
                        ` : ''}
                    </div>
                </td>
